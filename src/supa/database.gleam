...
pub fn execute_select(client: client.Client, query: Query, decoder: decode.Decoder(a), handler: fn(Result(List(a), rsvp.Error)) -> b) {
...
pub fn execute_select_single(client: client.Client, query: Query, decoder: decode.Decoder(a), handler: fn(Result(option.Option(a), rsvp.Error)) -> b) {
  let limited_query = limit(query, 1)
  execute_select(client, limited_query, decoder, fn(result) {
    case result {
...
pub fn execute_insert(client: client.Client, table: String, data: json.Json, handler: fn(Result(List(dynamic.Dynamic), rsvp.Error)) -> a) {
...
pub fn execute_insert_batch(client: client.Client, table: String, data: List(json.Json), handler: fn(Result(List(dynamic.Dynamic), rsvp.Error)) -> a) {
  let json_array = json.array(data, fn(x) { x })
  execute_insert(client, table, json_array, handler)
}

pub fn execute_upsert(client: client.Client, table: String, data: json.Json, handler: fn(Result(List(dynamic.Dynamic), rsvp.Error)) -> a) {
  let path = "/rest/v1/" <> table
...
pub fn execute_update(client: client.Client, query: Query, data: json.Json, handler: fn(Result(List(dynamic.Dynamic), rsvp.Error)) -> a) {
  let filter_strings = case query.filters {
    [] -> ""
...
pub fn execute_delete(client: client.Client, query: Query, handler: fn(Result(List(dynamic.Dynamic), rsvp.Error)) -> a) {
  let filter_strings = case query.filters {
    [] -> ""
    filters -> "?" <> string.join(list.map(filters, filter_to_string), "&")
  }
